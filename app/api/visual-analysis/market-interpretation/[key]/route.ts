...
export async function GET(_request: Request, context: { params: Promise<{ key: string }> }): Promise<Response> {
  try {
    ...
    const interpretation = await getLatestVisualMarketInterpretation(key);
    if (!interpretation) {
      return Response.json({ ok: false, error: 'Visual market interpretation was not found.' }, { status: 404, headers: { 'Cache-Control': 'no-store' } });
    }

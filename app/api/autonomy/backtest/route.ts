import { ensureStrategyBacktestingSchema, runStrategyBacktest } from '@/lib/strategy-backtesting';
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const result = await queryPostgres(
      `
        SELECT id, strategy_id, symbol, timeframe, mode, status, metrics_json, promotion_json, created_at
    ...
    return Response.json(
      { ok: false, error: error instanceof Error ? error.message : 'Unable to load backtest runs.' },
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const result = await runStrategyBacktest({
      ...
      decisionReplay: Boolean(body.decisionReplay ?? false),
      limit: Number(body.limit ?? 1200),
    });
    return Response.json({ ok: true, result }, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    return Response.json(
      { ok: false, error: error instanceof Error ? error.message : 'Unable to run strategy backtest.' },

...
export async function POST(request: Request, context: { params: Promise<{ accountNumber: string }> }): Promise<Response> {
  const { accountNumber } = await context.params;
  const body = await request.text();
  const response = await fetch(`${bridgeUrl()}/routing/accounts/${encodeURIComponent(accountNumber)}`, {
    method: 'POST',
    headers: {

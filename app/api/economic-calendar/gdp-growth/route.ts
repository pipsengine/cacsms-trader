...
function inferGrowthKind(normalizedName: string): GrowthKind {
  const s = normalizedName.toLowerCase();
  if (/\bgdp\b|\bgross domestic product\b/.test(s)) {
    if (/\bqoq\b|q\/q|quarter[-\s]?over[-\s]?quarter/.test(s)) return 'gdp_qoq';
    if (/\byoy\b|y\/y|year[-\s]?over[-\s]?year/.test(s)) return 'gdp_yoy';
    if (/\bannuali[sz]ed\b|\bar\b/.test(s)) return 'gdp_annualized';
    if (/\bdeflator\b/.test(s)) return 'gdp_deflator';
    if (/\bprice index\b/.test(s)) return 'gdp_price_index';
    return 'unknown';
  }
  if (/\bindustrial production\b|\bindustrial prod\b|\bmanufacturing production\b/.test(s)) return 'industrial_production';
  if (/\bpmi\b|\bpurchasing managers\b/.test(s)) return 'pmi';
  if (/\bretail sales\b/.test(s)) return 'retail_sales';
  return 'unknown';
}

function isReleased(row: GrowthEventRow): boolean {
  const actual = String(row.actual_value ?? '').trim();
  if (actual) return true;
  return ['RELEASED', 'ANALYZED', 'ARCHIVED'].includes(String(row.status ?? '').toUpperCase());
}

function isUpcoming(row: GrowthEventRow): boolean {
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const from = url.searchParams.get('from') ?? daysAgo(365 * 2);
    const to = url.searchParams.get('to') ?? daysAhead(120);
    ...
    const baseRows = await queryPostgres(
      ...
      `,
      params,
    ).then((r) => r.rows as unknown as GrowthEventRow[]);

    const decorated = baseRows
      .map((row) => ({
        ...row,
        kind: inferGrowthKind(String(row.normalized_event_name ?? '')),
    ...
    if (view === 'series') {
      ...
      const kind = String(url.searchParams.get('kind') ?? '').trim().toLowerCase() as GrowthKind;
      const series = decorated
        ...
        .map((row) => ({
          ...
          kind: (row as any).kind as GrowthKind,
          unit: row.unit,
    ...
    return NextResponse.json(
      { ok: false, generatedAt: new Date().toISOString(), error: error instanceof Error ? error.message : 'gdp_growth_failed', summary: null, upcoming: [], recent: [] },

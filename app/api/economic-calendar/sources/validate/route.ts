...
export async function POST(request: Request) {
  try {
    ...
    const result = await new EconomicCalendarIntelligenceService().recordAction('sources/validate');
    return NextResponse.json(result, { status: result.ok ? 202 : 503, headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    return NextResponse.json(
      { ok: false, message: error instanceof Error ? error.message : 'validate_failed' },

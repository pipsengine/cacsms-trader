...
function assertLocalOnly(request: Request) {
  ...
  if (forwardedFor || forwardedHost || forwardedProto) {
    throw new Error('Economic Calendar sync requires local machine access.');
  }
}
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    assertRateLimit('economic-calendar:ff:browser-sync', 60_000);
    const result = await new EconomicCalendarIntelligenceService().forexFactoryBrowserSync('manual_browser_sync');
    return Response.json({ ok: true, ...result }, { status: 202, headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    return Response.json(
      { ok: false, error: error instanceof Error ? error.message : 'Browser sync failed.' },

...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    assertRateLimit('economic-calendar:ff:hybrid-sync', 60_000);
    const result = await new EconomicCalendarIntelligenceService().forexFactoryHybridSync('manual_hybrid_sync');
    return Response.json({ ok: true, ...result }, { status: 202, headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    return Response.json(
      { ok: false, error: error instanceof Error ? error.message : 'Hybrid sync failed.' },

...
import { BarChart3, CalendarClock, Loader2, Menu, RefreshCw, Thermometer, TrendingDown, TrendingUp } from 'lucide-react';
...
type DashboardPayload = {
  ...
  upcoming: InflationEvent[];
  recent: InflationEvent[];
};
...
function parseNumber(value: string | null | undefined): number | null {
  ...
  if (!raw) return null;
  const cleaned = raw.replace(/,/g, '').replace(/[%]/g, '').replace(/[^\d.\-+]/g, '').trim();
...
function labelForKind(kind: InflationKind): string {
  if (kind === 'headline_yoy') return 'CPI YoY';
  if (kind === 'headline_mom') return 'CPI MoM';
  if (kind === 'core_yoy') return 'Core CPI YoY';
  if (kind === 'core_mom') return 'Core CPI MoM';
  if (kind === 'inflation_rate') return 'Inflation Rate';
  return 'Other';
}

const majorCurrencies = ['USD', 'EUR', 'GBP', 'JPY', 'CAD', 'AUD', 'NZD', 'CHF'] as const;

export default function CpiInflationEventsPage() {
  const [mobileSidebarOpen, setMobileSidebarOpen] = useState(false);
  const [dashboard, setDashboard] = useState<DashboardPayload | null>(null);
  const [series, setSeries] = useState<SeriesPayload | null>(null);
  const [selectedCurrency, setSelectedCurrency] = useState<string>('USD');
  const [selectedKind, setSelectedKind] = useState<InflationKind>('headline_yoy');
  ...
  const loadDashboard = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/economic-calendar/cpi-inflation', { cache: 'no-store' });
      const payload = await readJson<DashboardPayload>(res);
      setDashboard(payload);
      if (!payload.ok) setError(payload.error ?? 'Failed to load CPI/Inflation events.');
    } catch (e) {
      setDashboard(null);
      setError(e instanceof Error ? e.message : 'Failed to load CPI/Inflation events.');
    } finally {
      setLoading((s) => ({ ...s, dashboard: false }));
    }
  };

  const loadSeries = async (currency: string, kind: InflationKind) => {
    setLoading((s) => ({ ...s, series: true }));
    setError(null);
    try {
      const url = `/api/economic-calendar/cpi-inflation?view=series&currency=${encodeURIComponent(currency)}&kind=${encodeURIComponent(kind)}`;
      const res = await fetch(url, { cache: 'no-store' });
      const payload = await readJson<SeriesPayload>(res);
      setSeries(payload);
      if (!payload.ok) setError(payload.error ?? 'Failed to load CPI/Inflation history.');
    } catch (e) {
      setSeries(null);
      setError(e instanceof Error ? e.message : 'Failed to load CPI/Inflation history.');
    } finally {
    ...
    >
        <header className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 md:px-6 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-semibold tracking-tight text-slate-950">CPI / Inflation Events</h1>
              <p className="text-xs font-mono uppercase tracking-wider text-indigo-700">DB-first CPI, Core CPI, HICP and inflation releases from the internal economic calendar</p>
            </div>
          </div>
              ...
              <Card className="border-slate-200 bg-violet-50 shadow-sm shadow-slate-900/5">
                <CardHeader className="space-y-1 pb-3">
                  <CardTitle className="flex items-center gap-2 text-sm font-semibold text-slate-950">
                    <Thermometer className="h-4 w-4 text-violet-700" /> CPI Universe
                  </CardTitle>
                ...
                <CardHeader className="border-b border-slate-200">
                  <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-slate-500">Select a currency to view its CPI/inflation history.</div>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      {(['headline_yoy', 'headline_mom', 'core_yoy', 'core_mom', 'inflation_rate'] as InflationKind[]).map((k) => (
                        <Button
                          key={k}
                          type="button"
                          size="sm"
                ...
                <CardContent className="p-4">
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 xl:grid-cols-8">
                    {currencyCards.map((c) => {
                      ...
                      const lastActual = fmtValue(c.lastActual);
                      const lastForecast = fmtValue(c.lastForecast);
                      return (
                        <button
                          key={c.currency}
                          type="button"
                        ...
                        >
                          ...
                          <div className="font-mono text-xs text-slate-900">{lastActual}</div>
                          <div className="mt-1 text-[11px] text-slate-600">Forecast</div>
                          <div className="font-mono text-xs text-slate-700">{lastForecast}</div>
                        </button>
                      );
                    })}
                  </div>
            ...
            <section className="grid grid-cols-1 gap-4 xl:grid-cols-2">
              <Card className="border-slate-200 bg-white shadow-sm shadow-slate-900/5">
                <CardHeader className="border-b border-slate-200">
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      <div className="text-sm font-semibold text-slate-950">Inflation History</div>
                ...
                <CardContent className="p-4">
                  {loading.series ? (
                    ...
                    <div className="flex h-[260px] items-center justify-center text-sm text-slate-600">
                      No released CPI/inflation rows yet for {selectedCurrency}.
                    </div>
                  ) : (
                ...
                <CardHeader className="border-b border-slate-200">
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-slate-500">Upcoming and released CPI/inflation events from the stored calendar.</div>
                    </div>
                  ...
                  <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)}>
                    <TabsContent value="overview" className="m-0 p-4 space-y-3">
                      <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
                        This panel is fed entirely by the internal Economic Calendar tables. If the list is empty, open Economic Calendar Overview and run Discover Upcoming Events.
                      </div>
                        ...
                        <Table>
                          ...
                          <TableBody>
                            {loading.dashboard ? (
                              ...
                              <TableRow className="hover:bg-transparent">
                                <TableCell colSpan={7} className="h-20 text-center text-sm text-slate-600">
                                  No upcoming CPI/inflation events found in the current window.
                                </TableCell>
                              </TableRow>
                            ) : (
                        ...
                        <Table>
                          ...
                          <TableBody>
                            {loading.dashboard ? (
                              ...
                              <TableRow className="hover:bg-transparent">
                                <TableCell colSpan={9} className="h-20 text-center text-sm text-slate-600">
                                  No released CPI/inflation rows found in the current window.
                                </TableCell>
                              </TableRow>
                            ) : (
                              (dashboard?.recent ?? []).map((row) => {
                                ...
                                return (
                                  <TableRow key={row.id} className="hover:bg-slate-50">
                                    ...
                                    <TableCell className="px-3 py-2">
                                      {surprise == null ? <ToneBadge tone="slate">—</ToneBadge> : <ToneBadge tone={surpriseTone}>{fmtPct(surprise)}</ToneBadge>}
                                    </TableCell>
                                    <TableCell className="px-3 py-2">
                                      <ToneBadge tone={toneForImpact(row.impact_level)}>{row.impact_level}</ToneBadge>
                                    </TableCell>

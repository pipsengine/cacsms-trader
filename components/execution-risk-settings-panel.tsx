      ...
      <CardHeader className="border-b border-slate-100 py-4">
        <div className="flex items-center justify-between gap-3">
          <CardTitle className="flex items-center gap-2 text-sm font-semibold text-slate-900">
            ...
            Prop-firm risk — sizing, drawdown &amp; capacity
          </CardTitle>
      ...
      <CardContent className="space-y-5 p-4">
        ...
        <div className="rounded-md border border-emerald-200 bg-emerald-50/70 p-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-sm font-medium text-emerald-900">Continuous trading mode</p>
            <span
              className={cn(
                'rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase',
                settings?.continuousTradingEnabled ? 'bg-emerald-600 text-white' : 'bg-slate-200 text-slate-700',
              )}
            >
              {settings?.continuousTradingEnabled ? 'Active' : 'Off'}
            </span>
          </div>
          <p className="mt-1 text-xs text-emerald-800">
            {settings?.continuousTradingEnabled
              ? 'Trades until daily drawdown is exhausted. Overall drawdown, consecutive-loss, and trade-count stops are bypassed.'
              : 'Standard prop-firm stops apply (overall drawdown, consecutive losses, trade counts).'}
          </p>
          <div className="mt-3 grid gap-2 sm:grid-cols-3">
            <div className="rounded-md border border-emerald-100 bg-white px-3 py-2">
              <p className="text-[11px] uppercase tracking-wide text-slate-500">Daily budget left</p>
              <p className="mt-1 font-mono text-xl text-slate-950">
                ${settings?.remainingDailyLossAmount?.toFixed(2) ?? '—'}
              </p>
            </div>
            <div className="rounded-md border border-emerald-100 bg-white px-3 py-2">
              <p className="text-[11px] uppercase tracking-wide text-slate-500">Daily DD budget</p>
              <p className="mt-1 font-mono text-xl text-slate-950">
                ${settings?.dailyDrawdownBudgetUsd?.toFixed(0) ?? '—'}
              </p>
            </div>
            <div className="rounded-md border border-emerald-100 bg-white px-3 py-2">
              <p className="text-[11px] uppercase tracking-wide text-slate-500">Gate status</p>
              <p className="mt-1 text-sm font-medium text-slate-950">
                {(settings?.remainingDailyLossAmount ?? 0) > 0 ? 'Trading open' : 'Daily DD hit'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-md border border-violet-200 bg-violet-50/70 p-3">
          <p className="text-sm font-medium text-violet-900">Prop-firm lot-sizing equity</p>
          <p className="mt-1 text-xs text-violet-800">
            Reference ${settings?.propFirmReferenceEquity?.toFixed(2) ?? '—'} × max drawdown % → sizing base
            ${settings?.propFirmSizingEquity?.toFixed(2) ?? '—'}. Risk per position uses this envelope
            (${settings?.riskPerPositionUsd?.toFixed(2) ?? '—'} per slot).
          </p>
        </div>
        ...
        <div className="rounded-md border border-blue-200 bg-blue-50/70 p-3">
          ...
          <p className="mt-1 text-xs text-blue-800">
            ...
            divided by prop-firm risk per position (${settings?.riskPerPositionUsd?.toFixed(2) ?? '—'}).
          </p>
          <div className="mt-3 grid gap-2 sm:grid-cols-3">
        ...
        {settings?.continuousTradingEnabled ? (
          <div className="rounded-md border border-slate-200 bg-slate-50/80 p-3">
            <p className="text-sm font-medium text-slate-900">Trade count (monitoring only)</p>
            <p className="mt-1 text-xs text-slate-600">
              Daily trade limits are off in continuous mode. Only the daily drawdown budget and open-position capacity gate new entries.
            </p>
            <p className="mt-2 font-mono text-sm text-slate-800">
              Trades opened today: {settings.tradesOpenedToday ?? '—'}
            </p>
          </div>
        ) : (
          <>
            <div className="rounded-md border border-amber-200 bg-amber-50/60 p-3">
              ...
              <p className="mt-1 text-xs text-amber-800">
                {`Total daily trades = trades per symbol × active symbols (${settings?.activeSymbolCount ?? '—'}${settings?.activeSymbols?.length ? `: ${settings.activeSymbols.slice(0, 4).join(', ')}${settings.activeSymbols.length > 4 ? '…' : ''}` : ''}).`}
              </p>
              <div className="mt-3 grid gap-2 sm:grid-cols-3">

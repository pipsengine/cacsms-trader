...
import { Activity, ArrowLeft, FlaskConical, Gauge, Menu, Radio, Sparkles } from 'lucide-react';
...
import { useResearchEvolution } from '@/components/strategies/use-research-evolution';
...
import {
  getResearchEvolutionModule,
  RESEARCH_EVOLUTION_MODULES,
  type ResearchEvolutionSlug,
} from '@/lib/strategies/research-evolution-modules';
...
export function ResearchEvolutionDashboard({ moduleId }: { moduleId: ResearchEvolutionSlug }) {
  const moduleDef = getResearchEvolutionModule(moduleId)!;
  const [mobileSidebarOpen, setMobileSidebarOpen] = useState(false);
  const { loading, refreshing, error, payload, lastSyncAt } = useResearchEvolution(moduleId);
      ...
      <DashboardPageShell className="bg-slate-50">
        <header className="sticky top-0 z-20 flex min-h-14 shrink-0 flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white/90 px-4 py-3 backdrop-blur lg:px-6">
          <div className="flex items-center gap-3">
            ...
            <FlaskConical className={cn('h-5 w-5', toneBody(tone))} />
            <div>
              <p className={cn('text-[11px] font-semibold uppercase tracking-[0.18em]', toneBody(tone))}>Research & Evolution</p>
        ...
        <DashboardPageScroll className="space-y-4 p-4 lg:p-6">
          ...
          <Card className={cn('border shadow-sm', toneCard(tone))}>
            <CardContent className="flex flex-col gap-4 p-4 md:flex-row md:items-center md:justify-between">
              <div>
                <p className={cn('text-xs font-semibold uppercase tracking-wide', toneMuted(tone))}>Research algorithm</p>
                <p className={cn('mt-1 text-sm font-medium', toneBody(tone))}>{moduleDef.algorithm}</p>
                <p className={cn('mt-2 text-xs', toneMuted(tone))}>
                  Pipeline-driven · full book analysis · auto-refresh every {Math.round((payload?.refreshIntervalMs ?? 15000) / 1000)}s
              ...
              {result ? (
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <p className={cn('text-[10px] font-semibold uppercase tracking-wide', toneMuted(tone))}>Research signal</p>
          ...
          {result ? (
                  ...
                  <ScrollArea className="h-[min(420px,50vh)] pr-3">
                    <div className="space-y-2">
                      {result.rankings.map((row) => (
                        <StrategyRankingRow key={`${row.id}-${row.label}`} row={row} tone={tone} />
                      ))}
                      {result.rankings.length === 0 ? (
                        <p className={cn('text-sm', toneMuted(tone))}>No ranking rows for this research module yet.</p>
                      ) : null}
                    </div>
                  </ScrollArea>
                </CardContent>
            ...
            <Card className={cn('border shadow-sm', toneCard(tone))}>
              <CardContent className="flex items-center gap-3 p-6">
                ...
                <p className={cn('text-sm', toneBody(tone))}>Running research module across active book…</p>
              </CardContent>
            </Card>
          ) : null}
          ...
          <Card className={cn('border shadow-sm', toneCard('slate'))}>
            <CardHeader className={toneCardHeader('slate')}>
              <CardTitle className={cn('text-base', toneTitle('slate'))}>Research & Evolution modules</CardTitle>
            </CardHeader>
            <CardContent className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {RESEARCH_EVOLUTION_MODULES.map((item) => (
                <Link
                  key={item.id}
                  href={`/institutional-strategy-intelligence/research-and-evolution/${item.id}`}
                  className={cn(

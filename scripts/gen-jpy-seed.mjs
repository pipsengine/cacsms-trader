...
const text = readFileSync(resolve(__dirname, 'import-jpy-rate-history.mjs'), 'utf8');
...
const out = `/** Full JPY BoJ rate history from Investing.com event page 165 (Nov 2016 – Jun 2026). */\nexport const JPY_RATE_HISTORY_SEED = [\n${lines.join('\n')}\n] as const;\n`;

writeFileSync(resolve(__dirname, '../lib/rates/jpy-rate-history-seed-data.ts'), out);
console.log(`Wrote ${list.length} rows to lib/rates/jpy-rate-history-seed-data.ts`);

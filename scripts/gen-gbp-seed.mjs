...
const text = readFileSync(resolve(__dirname, 'import-gbp-rate-history.mjs'), 'utf8');
...
const out = `/** Full GBP BoE rate history from Investing.com event page 170 (Nov 2016 – Jun 2026). */\nexport const GBP_RATE_HISTORY_SEED = [\n${lines.join('\n')}\n] as const;\n`;

writeFileSync(resolve(__dirname, '../lib/rates/gbp-rate-history-seed-data.ts'), out);
console.log(`Wrote ${list.length} rows to lib/rates/gbp-rate-history-seed-data.ts`);

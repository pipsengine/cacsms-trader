...
const text = readFileSync(resolve(__dirname, 'import-eur-rate-history.mjs'), 'utf8');
...
const out = `/** Full EUR ECB rate history from Investing.com event page 164 (Jul 2016 – Jun 2026). */\nexport const EUR_RATE_HISTORY_SEED = [\n${lines.join('\n')}\n] as const;\n`;

writeFileSync(resolve(__dirname, '../lib/rates/eur-rate-history-seed-data.ts'), out);
console.log(`Wrote ${list.length} rows to lib/rates/eur-rate-history-seed-data.ts`);

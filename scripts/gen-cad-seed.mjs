...
const text = readFileSync(resolve(__dirname, 'import-cad-rate-history.mjs'), 'utf8');
...
const out = `/** Full CAD BoC rate history from Investing.com event page 166 (Sep 2015 – Jul 2026). */\nexport const CAD_RATE_HISTORY_SEED = [\n${lines.join('\n')}\n] as const;\n`;

writeFileSync(resolve(__dirname, '../lib/rates/cad-rate-history-seed-data.ts'), out);
console.log(`Wrote ${list.length} rows to lib/rates/cad-rate-history-seed-data.ts`);

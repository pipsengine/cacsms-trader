...
const text = readFileSync(resolve(__dirname, 'import-nzd-rate-history.mjs'), 'utf8');
...
const out = `/** Full NZD RBNZ rate history from Investing.com event page 167 (Aug 2016 – May 2026). */\nexport const NZD_RATE_HISTORY_SEED = [\n${lines.join('\n')}\n] as const;\n`;

writeFileSync(resolve(__dirname, '../lib/rates/nzd-rate-history-seed-data.ts'), out);
console.log(`Wrote ${list.length} rows to lib/rates/nzd-rate-history-seed-data.ts`);

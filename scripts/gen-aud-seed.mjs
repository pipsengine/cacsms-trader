...
const text = readFileSync(resolve(__dirname, 'import-aud-rate-history.mjs'), 'utf8');
...
const out = `/** Full AUD RBA rate history from Investing.com event page 171 (Oct 2017 – Jun 2026). */\nexport const AUD_RATE_HISTORY_SEED = [\n${lines.join('\n')}\n] as const;\n`;

writeFileSync(resolve(__dirname, '../lib/rates/aud-rate-history-seed-data.ts'), out);
console.log(`Wrote ${list.length} rows to lib/rates/aud-rate-history-seed-data.ts`);

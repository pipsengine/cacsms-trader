...
const text = readFileSync(resolve(__dirname, 'import-chf-rate-history.mjs'), 'utf8');
...
const out = `/** Full CHF SNB rate history from Investing.com event page 169 (Sep 2016 – Jun 2026). */\nexport const CHF_RATE_HISTORY_SEED = [\n${lines.join('\n')}\n] as const;\n`;

writeFileSync(resolve(__dirname, '../lib/rates/chf-rate-history-seed-data.ts'), out);
console.log(`Wrote ${list.length} rows to lib/rates/chf-rate-history-seed-data.ts`);
